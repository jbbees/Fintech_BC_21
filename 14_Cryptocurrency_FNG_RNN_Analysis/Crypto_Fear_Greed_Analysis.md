...
It could be the RNN weighted negative sentiment more in the interpretability of model pricing, and that led the model to predict much lower closing prices to persist throughout the model. Additionally, the sentiment data could be heavily imbalanced with more negative sentiment values than positive. If there's negative sentiment, there's likely low closing prices, and positive sentiment will be correlated with higher closing prices. I can surmise there was likely crypto rallies trying to drive crypto values up in the later half, and that's why we see the sharp increase of prices at the later part of the models. Usually when BTC declines, ambitious investors buy in the dip and begin rallying new investors into the space, hence we'd expect more positive sentiment. But since predicted prices were much lower in the price spike, that strengthens my belief this sentiment data is biased/imbalanced, could have more negative sentiment in the rolling-window being fed into it. So the sentiment data is imbalanced with more negativity of Bitcoin.  

I would never use sentiment scores to predict crypto values. This is worse than using direct prices that are almost never static.
